#!/bin/bash
# Chancellor startup script — prefers kingdom-session-manager, falls back to legacy nats-bridge.
set -e

WORKSPACE=/workspace
LOG=$WORKSPACE/nats-bridge.log

echo "[start.sh] Chancellor starting up..."
...
start_process() {
  if [ -f "$SESSION_MGR" ]; then
    ...
    AGENT_ROLE=chancellor NATS_URL="${_SM_NATS_URL}" node "$SESSION_MGR" >> $LOG 2>&1 &
    ACTIVE_PID=$!
